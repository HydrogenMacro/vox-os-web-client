...
export function VoiceVisualizer({ segmentsCount }: { segmentsCount: number }) {
    ...
    useEffect(() => {
        ...
        function drawLoop() {
            ...
            for (let i = 0; i < segmentsCount; i++) {
                const segmentHeight = micAudio[Math.floor(i / segmentsCount * micAudio.length)] / 255 * (cvsHeight - segmentWidth);
                ctx.beginPath();
                ctx.arc(x + segmentWidth / 2, cvsHeight / 2 - segmentHeight / 2, segmentWidth / 2, 0, Math.PI, true);
                /*
                ctx.fillRect(
                    x,
                ...
                ctx.lineTo(x, cvsHeight / 2 + segmentHeight / 2);
                ctx.arc(x + segmentWidth / 2, cvsHeight / 2 + segmentHeight / 2, segmentWidth / 2, 0, Math.PI);
                ctx.lineTo(x + segmentWidth, cvsHeight / 2 + segmentHeight / 2);
                ctx.closePath();
                ctx.fill();
